...
| 22 | **Field exposure and lost E17 pairs** | **Validity: yes.** Corpus recovery shows that two of 25 D-SSNs were absent from the fine-tuning text. The current D/C analysis therefore reports a disclosed post-result exclusion of those two D targets and two C targets. The executed E3 path discarded E17's target-level pairs when it deduplicated and capped control people; one original paired C was never attacked, so the correction uses the nearest actually attacked C-SSN under the same three E17 covariates. The resulting 48/48 H3/H5 estimates supersede the unfiltered D/C interpretation, but remain conditional and post hoc. |
